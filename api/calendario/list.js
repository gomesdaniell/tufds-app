...
export default async function handler(req, res) {
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      const tipoLinhaNorm = norm(tipoLinhaRaw);
      ...
      // - é gira
      // - OU tem atividade
      // - OU tem feriado/comemoração
      const isGira = (tipoLinhaNorm === 'gira');
      const hasAtividade = !!atividade;
      const hasFeriado = !!feriado;

      if (!isGira && !hasAtividade && !hasFeriado) continue;

      const { dataStr, iso } = fmtDate(dt, TZ);
      const mes = iso.slice(5,7), ano = iso.slice(0,4);

      let categoria = 'Evento';
      if (isGira) categoria = 'Gira';
      else if (hasAtividade) categoria = 'Atividade';
      else if (hasFeriado) categoria = 'Feriado/Comemoração';

      // Título padrão (pra UI)
      let titulo = '';
      if (isGira) titulo = atividade || (linha ? `Gira – ${linha}` : 'Gira');
      else titulo = atividade || feriado || 'Evento';
